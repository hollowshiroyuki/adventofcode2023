...
int main(void) {
  ...
  for (int y = 0; y < HEIGTH; y++) {                    // traverse map y
    for (int x = 0; x < WIDTH; x++) {                   // traverse map x
      ...
      if (pos == '*') { // check that pos is a "gear"
        int ratio = 1;
        int part_numbers_count = 0;
            ...
            if (isdigit(*c)) {      // if adjacent is a number
              for (number_t *cur = numbers; cur;
                ...
                if (cur->start <= c &&
                  ...
                  ratio *= cur->value;  // multiply ratio
                  ++part_numbers_count; // increase the part count encountered
                }
              }
            }
          }
        }
        if (part_numbers_count == 2) // a valid gear only has 2 adjacent numbers
          total += ratio;            // add the ratio to the total
      }
    }
  }
  printf("%lu\n", total);
